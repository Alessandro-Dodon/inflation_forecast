################################################################################
# Script Overview: Pre-Processing and Visualization
################################################################################

# Author: Alessandro Dodon
# Last Update: 12-12-2024
# Description:
# This script performs an extensive analysis of macroeconomic data, specifically
# targeting inflation (CPIULFSL) as the variable of interest. It includes data 
# preprocessing, transformation, cleaning, visualization, and eigenvalue analysis. 
# The script also saves key plots and outputs for further review.

# Requirements:
# - The script requires R and the listed packages to be installed.
# - To install the necessary packages, remove the `#` in the package installation 
#   commands under the "Prepare packages" section.
# - A CSV file named `current.csv` containing the dataset should be in the same 
#   directory as this script.

# Major Sections:
# 1. Prepare Packages: Installs and loads all required packages for the analysis.
# 2. Load Data and Initial Inspection: Reads and preprocesses the dataset with the fredmd function.
# 3. Describing Variables: Loads and inspects metadata for better understanding.
# 4. Handling Missing Values: Identifies, counts, and handles missing values in the data.
# 5. Data Cleaning: Filters rows and columns based on specific criteria, such as date range and missing data.
# 6. Removing COVID-19 Period: Excludes data during the COVID-19 years for analysis.
# 7. Data Splitting: Uses the holdout method to create training and testing datasets.
# 8. Variable of Interest (CPIULFSL): Focuses on inflation as the target variable and provides detailed visualizations.
# 9. Visualization of All Variables: Creates and saves time series and autocorrelation plots for all variables.
# 10. Covariance, Correlation, and PCA: Computes and visualizes covariance, correlation, eigenvalues, and eigenvectors for dimensionality reduction.
# 11. Eigenvalue Ratios: Creates scree plots to analyze the ratios of eigenvalues.

# Outputs:
# - Key plots, including time series, autocorrelation, heatmaps, and scree plots, are saved in the 
#   working directory as PDF or PNG files for easy access and review.

# Notes:
# - Ensure all required packages are installed before running the script.
# - Ensure the CSV file is located in the same directory as this R script.
...
################################################################################
# Function to create a plot for variable of interest (CPIULFSL, target inflation variable)
################################################################################
...
################################################################################
# Plotting before and after ALL time series
################################################################################
...
################################################################################
# Plotting before and after the autocorrelation of ALL time series
################################################################################
