...
# Description:
# This script evaluates the performance of Lasso regression models using various 
# lambda values. It performs recursive predictions to forecast the 
# variable CPIULFSL (inflation proxy). The results include error metrics 
# (MSE, MAE, RMSE) and plots, such as Actual vs Predicted, Residual plots, 
# and MSE vs Lambda. The script saves all plots to a specified folder.
...
# - Plots saved in the specified folder (`lasso_results_folder`):
...
# Function to test multiple lambdas, save plots to a folder, and print all metrics for Lasso
test_lasso_lambdas <- function(lambda_values, title_prefix, output_folder) {
  ...
  for (i in seq_along(lambda_values)) {
    ...
    # Run Lasso regression for this lambda
    ...
    residual_plot <- ggplot(df_test, aes(x = Actual, y = Actual - Predicted)) +
      ...
      ggtitle(paste("Lasso Regression: Residual Plot (Lambda =", format(lambda, scientific = TRUE), ")")) +
    ...
    scatter_plot <- ggplot(df_test, aes(x = Actual, y = Predicted)) +
      ...
      ggtitle(paste("Lasso Regression: Actual vs Predicted (Lambda =", format(lambda, scientific = TRUE), ")")) +
...
results <- test_lasso_lambdas(lambda_values, "Lasso Regression", "lasso_results")
